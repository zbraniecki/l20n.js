(function(global) {
    ...
    while (true) {
      ...
      if (considerDefaultLocale && candidate.length <= defaultLocale.length) {
        if (candidate === defaultLocale)
          return candidate;
        if (defaultLocale.startsWith(candidate + "-"))
          return candidate;
      }
  ...
  function removeUnicodeExtensions(locale) {
    ...
    var left = locale.substring(0, pos);
    var right = locale.substring(pos);
  ...
  function CanonicalizeLocaleList(locales) {
    ...
    locales.forEach(languageTag => {
      let tag = CanonicalizeLanguageTag(languageTag);
      result.add(tag);
    });
    return result;
  }
  ...
  function PrioritizeAvailableLocales(availableLocales, requestedLocales,
      defaultLocale) {
