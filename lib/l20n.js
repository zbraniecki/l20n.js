(function() {
    ...
    function importFirstURL(urls, callback, fallback, nesting, async) {
      ...
      imported.load(
        function importSucceeded() {
          ...
          if (performanceTimer) {
            var end = new Date();
            var start = ctx.perfData.timers[ctx.id][url]['start'];
            ctx.perfData['resloading'].push(end-start);
          }
          callback();
        }, 
  ...
  function Context() {
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      if (performanceTimer) {
        if (!this.perfData.timers) {
          this.perfData.timers = {}
        }
        if (!this.perfData.timers[self.id]) {
          this.perfData.timers[self.id] = {}
        }
        this.perfData.timers[self.id][uri] = {'start': new Date(), 'end': null}; 
      }
      uris.push(uri);
