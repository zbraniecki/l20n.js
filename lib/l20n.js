(function() {
  ...
  L20n.getContext = function L20n_getContext(id) {
    ...
    var ctx = new Context();
    if (window.performanceTimer) {
      performanceTimer.addDataPoint(id, 'bootstrap', null, start);
      ctx.id = id;
    }
    return ctx;
  };
  ...
  function Context() {
    ...
    function compile() {
      ...
      if (window.performanceTimer) {
        performanceTimer.addDataPoint(self.id, 'compilation', null, start); 
      }
      isReady = true;
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      if (window.performanceTimer) {
        if (!performanceTimer.timers[uri]) {
          performanceTimer.setTimerCallback(uri, 'resloading', function(start, end) {
            performanceTimer.addDataPoint(self.id, 'resloading', uri, start, end);
          });
          performanceTimer.setTimerCallback(uri, 'parsing', function(start, end) {
            performanceTimer.addDataPoint(self.id, 'parsing', uri, start, end);
          });
        }
      }
      uris.push(uri);
    ...
    this.get = function ctx_get(id, data, callback, fallback) {
      if (callback === undefined) {
        ...
        console.log('asking for entity: '+id);
        var d = getSync(id, data);
        console.log('returning value: '+d);
        if (window.performanceTimer) {
