(function() {
  ...
  L20n.getContext = function L20n_getContext(id) {
    ...
    if (performanceTimer) {
      ...
      performanceTimer.addDataPoint(id, 'bootstrap', null, end-start);
      ctx.id = id;
    }
    return ctx;
  };
  ...
  function Resource(id) {
    ...
    function parse(data) {
      ...
      if (performanceTimer) {
        var end = performanceTimer.getTime();
        performanceTimer.resolveTimer(id, 'parsing', end-start);
      }
  ...
  function Context() {
    ...
    function compile() {
      ...
      performanceTimer.addDataPoint(self.id, 'compilation', null, performanceTimer.getTime() - start); 
      isReady = true;
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      if (performanceTimer) {
        if (!performanceTimer.timers[uri]) {
          performanceTimer.setTimerCallback(uri, 'resloading', function(time) {
            performanceTimer.addDataPoint(self.id, 'resloading', uri, time);
          });
          performanceTimer.setTimerCallback(uri, 'parsing', function(time) {
            performanceTimer.addDataPoint(self.id, 'parsing', uri, time);
          });
        }
      }
      uris.push(uri);
    ...
    this.get = function ctx_get(id, data, callback, fallback) {
      if (callback === undefined) {
        ...
        performanceTimer.addDataPoint(self.id, 'execution', id, performanceTimer.getTime() - start);
        return d;
      }
