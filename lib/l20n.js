(function() {
  ...
  if (typeof exports !== 'undefined') {
    L20n = exports;
    L20n.EventEmitter = require('./events.js');
  ...
  L20n.getContext = function L20n_getContext(id) {
    ...
    if (performanceTimer) {
      ...
      performanceTimer.ensureContext(id);
      performanceTimer.perfData.contexts[id]['bootstrap'].push(end-start);
      ctx.perfData = performanceTimer.perfData['contexts'][id];
      ctx.id = id;
    }
    return ctx;
  };
  ...
  function Resource(id) {
    ...
    function downloadAsync(url, callback) {
      ...
      xhr.addEventListener('load', function() {
        if (xhr.status == 200) {
          ...
          if (performanceTimer) {
            var end = performanceTimer.getTime();
            var start = performanceTimer.timers[url]['load'].done(end);
          }
      ...
      if (performanceTimer) {
        xhr.addEventListener('readystatechange', function(e) {
          if (xhr.readyState == 2) {
            performanceTimer.timers[url]['load'].start = performanceTimer.getTime();
          }
        });
      }
    ...
    function parse(data) {
      isDownloading = false;
      if (!L20n.parser) {
        L20n.parser = new L20n.Parser(L20n);
      }
      ...
      try {
        ast = L20n.parser.parse(data, true).body;
      } catch (e) {
      ...
      if (performanceTimer) {
        var end = performanceTimer.getTime();
        performanceTimer.timers[id]['parsing'].done(end-start);
      }
  ...
  function Context() {
    ...
    function compile() {
      ...
      try {
        ...
        if (!L20n.Compiler) {
          debug('L20n.Compiler is not ready!');
        }
      ...
      self.perfData['compilation'].push(performanceTimer.getTime() - start);
      isReady = true;
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      if (performanceTimer) {
        if (!performanceTimer.timers[uri]) {
          performanceTimer.timers[uri] = {
            'load': {
              'done': function(end) {
                var start = performanceTimer.timers[uri]['load'].start;
                self.perfData['resloading'][uri] = end - start;
              },
              'start': null,
            },
            'parsing': {
              'done': function(time) {
                self.perfData['parsing'][uri] = time;
              },
            },
          }
        }
    ...
    this.get = function ctx_get(id, data, callback, fallback) {
      if (callback === undefined) {
        ...
        self.perfData['execution'][id] = performanceTimer.getTime() - start;
        return d;
      }
