...
function PerfTest() {
  ...
  this.perfData = {
    'doc': {
      'performance.*': {}, // performance API numbers
      'dom.loaded': [], // time between navifationStart and DOMContentLoaded
      'doc.loaded': [], // time between navigationStart and window.load
      'doc.time': [], // time when window.load has been fired
      'DocumentLocalized': [], // time between navigationStart and DocumentLocalized
      'l10n ctx ready': [],
      'pages': {},
      'node l10n': {},
    },
  ...
  this.ensureContext = function(id) {
    if(!this.perfData.contexts[id]) {
      this.perfData.contexts[id] = {
        'l10n': [], // time between l10n init and firing of DocumentLocalized
        'bootstrap': [], // code that initalizes localization
        'resloading (sync)': {}, // time that takes to load l10n resources
        'resloading (async)': {}, // time that takes to load l10n resources
        'parsing': {}, // time that takes to parse l10n resources
        'compilation': [], // time that takes to compile l10n resources
        'execution': {} // time that takes to localize nodes
      }; 
    }
  }
...
window.addEventListener('load', function() {
    window.performanceTimer.addDataPoint(null, 'doc.loaded', null, 0);
    window.performanceTimer.addDataPoint(null, 'doc.time', null, 0, new Date().getTime());
    init();
});
document.addEventListener('DOMContentLoaded', function() {
    window.performanceTimer.addDataPoint(null, 'dom.loaded', null, 0);
});
